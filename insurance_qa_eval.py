    ...
    def get_score(self, verbose=False):
        for name, data in self.eval_sets().items():
            ...
            for i, d in enumerate(data):
                ...
                #print(indices)
                #p  rint(self.answers.keys())

                #print
               # print([self.answers[i] for i in indices])
                ...
                tst_indices = [0]
               # print([self.answers.keys()])
                answers_test = self.pada([self.answers[i] for i in self.answers.keys()])
                #print(answers_test)
                question = self.padq([d['question']] * len(indices))
              #  print(d['question'])
              #  print(question)
               # quit()
                #sims = self.model.predict([question, answers_test])
...
if __name__ == '__main__':
    ...
    #print('best loss epoc '+ best_loss['epoch'])
    #evaluator.load_epoch(best_loss['epoch'])
